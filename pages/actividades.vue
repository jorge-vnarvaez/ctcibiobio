<template lang="">
    <v-container class="py-10">
        ...
        <div class="text-subtitle">Conoce los hitos y actividades del proyecto</div>
            ...
            <v-timeline dense clipped align-top v-if="data">
                <v-timeline-item
                    ...
                    <v-card flat class="mt-n4">
                        <v-card-title :class="`${(milestone.parent_milestone) ? 'text-h6' : 'text-h5'}`">
                            {{ milestone.title }}
                        </v-card-title>
                        <v-card-subtitle>
                        ...
                        <v-card-text v-if="milestone.excerpt" class="text-body-1">{{ milestone.excerpt }}</v-card-text>
                        <v-card-text v-if="milestone.body" v-html="milestone.body"></v-card-text>
                        <v-data-table
                            ...
                            <td :colspan="headers.length" class="py-10">
                                ...
                                <div v-if="item.gallery.length">
                                    <div class="d-flex flex-wrap gap-3 py-5">
                                        <v-img class="black "v-for="(galleryfile, index) in item.gallery" :key="index+'g'" 
                                        :src="`${$config.apiAssetsV2+galleryfile.directus_files_id.filename_disk}`" 
                                        width="300px" max-width="300px" height="300px" contain>
                                        </v-img>
                                    </div>
                                </div>
                            </td>
                        ...
                        <template v-slot:item.date_start="{ item }">
                            <span>
                                <span v-if="item.date_start">{{ $moment(item.date_start).format('DD/MM/YY') }}</span>
                                <span v-else>-</span>
                            </span>
                        </template>              
                        ...
                        <template v-slot:item.date_end="{ item }">
                            <span>
                                <span v-if="item.date_end">{{ $moment(item.date_end).format('DD/MM/YY') }}</span>
                                <span v-else>-</span>
                            </span>
                        </template>                        
...
export default {
    ...
    async fetch() {
        ...
        this.data = await this.$axios.$get(`https://apificr.uinn.cl/items/milestones`, {
            params: {
                fields: [
                    '*',
                    'activities.*',
